...
import {
    ...
    CardHeader, Card, Icon, IconButton, Avatar, CardActions, TextField, CardMedia
...
class MyPostCard extends Component {
    ...
    fetchPostComments(check) {
        const { post, commentReducer } = this.props;
        if (!this.state.expanded || (check && check.action === "ADD_COMMENT")) {
    ...
    addComment() {
        const { post, onGetMyPosts, postsReducer, user } = this.props
        const { commentMessage } = this.state;
        if (!commentMessage && commentMessage.length === 0) return toastr.warning("Please enter comment to add.");
        postsReducer.commentPost({
            postId: post.postId,
            commentBy: {
                email: user.email, displayName: user.displayName, commentMessage,
            }
        }).then(res => res.success ? this.setState({ commentMessage: '' }, () => {
            this.fetchPostComments({ action: "ADD_COMMENT" });
            onGetMyPosts();
        }) : null
        );
    }
        ...
        return (
            <div key={post.postId}>
                <Card style={{ padding: "20px 20px 0px 20px", marginBottom: '20px' }}>
                    ...
                    <div>
                        <EditPost open={this.state.openEdit} handleClose={this.handleClose} />
                    </div>
                    <div>
                        <HidePost open={this.state.openHide} handleClose={this.handleClose} />
                    </div>
                    <div>
                        <DeletePost open={this.state.openDelete} handleClose={this.handleClose} deletePost={this.deletePost} />
                    </div>
                </Card>
            </div>
        )
